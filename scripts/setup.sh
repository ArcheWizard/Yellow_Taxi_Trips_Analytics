#!/bin/bash
# Yellow Taxi Analytics - Simplified Setup Script
# One script for all common operations
...
echo ""
echo "  ${GREEN}1${NC} - Initial setup (database + sample data 93K rows)"
echo "  ${GREEN}2${NC} - Load full dataset (17.4M rows)"
echo "  ${GREEN}3${NC} - Refresh materialized views (incremental)"
echo "  ${GREEN}4${NC} - Setup daily cron job for MV refresh"
echo "  ${GREEN}5${NC} - Start API server (FastAPI)"
echo "  ${GREEN}6${NC} - Run benchmark (incremental refresh)"
echo "  ${GREEN}7${NC} - Test API endpoints"
echo "  ${GREEN}8${NC} - Check database status"
...
case $choice in
    ...
    2)
        echo -e "\n${YELLOW}Loading full dataset (17.4M rows)...${NC}\n"
    ...
    4)
        echo -e "\n${YELLOW}Setting up daily cron job...${NC}\n"
    ...
    8)
        echo -e "\n${YELLOW}Checking database status...${NC}\n"
        source .env
